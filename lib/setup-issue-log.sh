...
: "${HOME:=/root}"
# _LINUX_LOG_PATH_FIX_2026-05-30 -- ~/Library/Logs is a macOS convention. On Linux
# use the XDG state dir so logs land where a Linux user expects them.
if [[ "$(uname -s)" == "Linux" ]]; then
  PBOX_LOG_DIR="${XDG_STATE_HOME:-$HOME/.local/state}/pandoras-box/logs"
else
...
pbox_claude_help() {
  ...
  [[ "${PBOX_DRY_RUN_ACTIVE:-0}" == "1" || "${PBOX_UNATTENDED_ACTIVE:-0}" == "1" ]] && return 0
