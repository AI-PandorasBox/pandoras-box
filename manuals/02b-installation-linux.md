...
- A **Claude Pro or Max subscription** (claude.ai), signed in via `claude /login`. API-key (pay-per-token) billing is not supported in this release; API support is planned for a future version.
