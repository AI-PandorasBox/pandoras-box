...
3. **Claude credentials.** API key OR subscription auth via `claude /login` (your choice). Stored in the macOS Keychain (on Linux, an encrypted file under `/opt/pandoras-box/.secrets`).
...
**Prerequisites (macOS):** macOS 14 or later, Node.js 20+, Homebrew, an Anthropic account (Claude Pro or Pro Max recommended).

**Prerequisites (Linux):** Debian 13 (Trixie) or Ubuntu 24.04 LTS or later with systemd, Node.js 20+, a `sudo` user, an Anthropic account. No Homebrew (the installer uses `apt`). Full-disk encryption (LUKS) is recommended and set at OS-install time. Full walkthrough: [manuals/02b-installation-linux.md](manuals/02b-installation-linux.md).
