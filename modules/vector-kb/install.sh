...
SERVICE_USER="${VECTOR_KB_USER:-$(pbox_stat_owner "$INSTALL_PATH")}"
if [[ "$PBOX_OS" == Darwin ]]; then
  ...
  if [[ "$DRY_RUN_ACTIVE" == "1" ]]; then
    ...
    VK_LOG="/tmp/${LOG_PREFIX:-pandoras-box}-vector-kb.log"
    pbox_create_service "$PLIST_LABEL" "$NODE_BIN" "$TARGET_DIR/$RUNTIME_SCRIPT" \
      "$SERVICE_USER" "$VK_LOG" "$TARGET_DIR" "$ENV_PATH" || fail "systemd service install failed"
