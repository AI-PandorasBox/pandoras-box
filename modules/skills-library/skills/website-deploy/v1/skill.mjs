// website-deploy — packaged skill (promoted from recipe spec) _SKILL_PROMOTED_V1
...
// Risk: medium. Invocation: conversational.
//
...
function assertCtx (ctx) {
  if (!ctx || typeof ctx.tool !== 'function') {
    throw new Error('website-deploy: ctx.tool(name,args) is required');
  }
  const missing = REQUIRED_TOOLS.filter(t => ctx.available && !ctx.available.includes(t));
  if (ctx.available && missing.length) {
    throw new Error('website-deploy: required tools not available: ' + missing.join(', '));
  }
}

export default async function website_deploy (input = {}, ctx = {}) {
  assertCtx(ctx);
  const log = ctx.log || (() => {});
  const steps = [];
  log('[website-deploy] start (risk=medium)');
  ...
  log('[website-deploy] done — ' + steps.length + ' steps');
  return { ok: true, skill: 'website-deploy', steps };
}
