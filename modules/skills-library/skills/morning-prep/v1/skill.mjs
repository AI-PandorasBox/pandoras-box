// morning-prep — packaged skill (promoted from recipe spec) _SKILL_PROMOTED_V1
...
function assertCtx (ctx) {
  if (!ctx || typeof ctx.tool !== 'function') {
    throw new Error('morning-prep: ctx.tool(name,args) is required');
  }
  const missing = REQUIRED_TOOLS.filter(t => ctx.available && !ctx.available.includes(t));
  if (ctx.available && missing.length) {
    throw new Error('morning-prep: required tools not available: ' + missing.join(', '));
  }
}

export default async function morning_prep (input = {}, ctx = {}) {
  assertCtx(ctx);
  const log = ctx.log || (() => {});
  const steps = [];
  log('[morning-prep] start (risk=low)');
  ...
  log('[morning-prep] done — ' + steps.length + ' steps');
  return { ok: true, skill: 'morning-prep', steps };
}
