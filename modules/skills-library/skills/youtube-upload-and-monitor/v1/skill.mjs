// youtube-upload-and-monitor — packaged skill (promoted from recipe spec) _SKILL_PROMOTED_V1
...
function assertCtx (ctx) {
  if (!ctx || typeof ctx.tool !== 'function') {
    throw new Error('youtube-upload-and-monitor: ctx.tool(name,args) is required');
  }
  const missing = REQUIRED_TOOLS.filter(t => ctx.available && !ctx.available.includes(t));
  if (ctx.available && missing.length) {
    throw new Error('youtube-upload-and-monitor: required tools not available: ' + missing.join(', '));
  }
}

export default async function youtube_upload_and_monitor (input = {}, ctx = {}) {
  assertCtx(ctx);
  const log = ctx.log || (() => {});
  const steps = [];
  log('[youtube-upload-and-monitor] start (risk=high)');
  ...
  log('[youtube-upload-and-monitor] done — ' + steps.length + ' steps');
  return { ok: true, skill: 'youtube-upload-and-monitor', steps };
}
