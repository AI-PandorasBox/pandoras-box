// marketing-channel-post — packaged skill (promoted from recipe spec) _SKILL_PROMOTED_V1
...
function assertCtx (ctx) {
  if (!ctx || typeof ctx.tool !== 'function') {
    throw new Error('marketing-channel-post: ctx.tool(name,args) is required');
  }
  const missing = REQUIRED_TOOLS.filter(t => ctx.available && !ctx.available.includes(t));
  if (ctx.available && missing.length) {
    throw new Error('marketing-channel-post: required tools not available: ' + missing.join(', '));
  }
}

export default async function marketing_channel_post (input = {}, ctx = {}) {
  assertCtx(ctx);
  const log = ctx.log || (() => {});
  const steps = [];
  log('[marketing-channel-post] start (risk=high)');
  ...
  log('[marketing-channel-post] done — ' + steps.length + ' steps');
  return { ok: true, skill: 'marketing-channel-post', steps };
}
