...
> **Per-tenant files task agent**
>
> **Status:** Available now. Auto-installed per-tenant by `setup-company.sh`. NOT operator-selectable in the main module menu.

The files task agent runs once per tenant. It polls the per-tenant `<slug>/store/jobs.db` for APPROVED jobs of `task_type='files'`, executes them via `@anthropic-ai/claude-agent-sdk`, and writes results back to the same DB. Operates under the tenant's service-account UID.
...
- Google / Google Drive (`GOOGLE_CLIENT_ID` set): **preview, not functional** in this release. No Google MCP server ships yet, so the agent cannot act on Google Drive even when these credentials are present.
...
- Loaded iff `FILES_ENABLED=true` in the company `.env`
