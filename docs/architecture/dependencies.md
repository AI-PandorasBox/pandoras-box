...
- **personal-ai (Personal AI's memory daemon)** — reads from + writes to the shared SQLite memory DB. All agents that consult shared memory depend on it.
...
| **personal-ai (Personal AI memory daemon)** | Personal AI offline. All cross-tenant memory reads fail. Business agents that consult shared memory degrade to "no memory" mode. |
...
| **Recall scoring** | Loss of admin visibility into recall scoring. Actual recall continues uninterrupted (scoring runs inside personal-ai; the panel is read-only). |
...
| Bridge | The bridge restarts on the next chat session boot. If stuck, restart personal-ai. |
...
- **personal-ai healthy** = Personal AI works. Business tenants still operate without it.
...
| 4 | personal-ai (Personal AI), trading-research (if installed) |
...
The `personal-ai` daemon also writes structured logs to its own store, viewable in the Personal AI UI's Admin tab.
